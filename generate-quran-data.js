// سكريبت توليد ملف بيانات القرآن الكامل
// شغل: node generate-quran-data.js
...
async function generateQuranData() {
    console.log('🚀 بدء تحميل بيانات القرآن الكريم...\n');
    
    const quranData = {
        meta: {
            source: 'مصحف وليد الإلكتروني',
            version: '1.0.0',
    ...
    try {
        // تحميل كل سورة
        for (let i = 1; i <= 114; i++) {
            console.log(`📥 تحميل سورة ${surahNames[i-1]} (${i}/114)...`);
                ...
                if (uthmaniData.code === 200 && imlaeiData.code === 200) {
                    ...
                    uthmaniAyahs.forEach((ayah, idx) => {
                        surah.ayahs.push({
                            ...
                            aya_text: ayah.text, // النص العثماني
                            aya_text_emlaey: imlaeiAyahs[idx].text, // النص الإملائي
                            page: ayah.page,
                            juz: ayah.juz,
                    ...
                    console.log(`   ✅ تم تحميل ${surah.ayahs.length} آية`);
                }
            } catch (e) {
                console.log(`   ❌ خطأ في تحميل السورة ${i}: ${e.message}`);
            }
        }
        ...
        console.log(`📖 السور: ${quranData.surahs.length}`);
